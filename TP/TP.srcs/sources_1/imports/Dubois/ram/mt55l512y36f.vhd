...
ARCHITECTURE behave OF mt55l512y36f IS
...
BEGIN
    ...
    main : PROCESS
        ...
        VARIABLE ce_in : STD_LOGIC := '0';
        VARIABLE rw_in : STD_LOGIC_VECTOR (1 DOWNTO 0) := "11";
        VARIABLE bwa_in : STD_LOGIC_VECTOR (1 DOWNTO 0) := "00";
        VARIABLE bwb_in : STD_LOGIC_VECTOR (1 DOWNTO 0) := "00";
        VARIABLE bwc_in : STD_LOGIC_VECTOR (1 DOWNTO 0) := "00";
        VARIABLE bwd_in : STD_LOGIC_VECTOR (1 DOWNTO 0) := "00";
        ...
        IF Clk'EVENT AND Clk = '1' THEN
            IF Cke_n = '0' AND Zz = '0' THEN
                ...
                IF rw_in (0) = '0' AND bwc_in (0) = '0' THEN
                    bank2 (CONV_INTEGER (addr_write)) := Dq (26 DOWNTO 18);
                END IF;
                IF rw_in (0) = '0' AND bwd_in (0) = '0' THEN
                    bank3 (CONV_INTEGER (addr_write)) := Dq (35 DOWNTO 27);
                END IF;
            END IF;

            -- Read Data from Memory Array
            IF ce_in = '1' AND rw_in (1) = '1' THEN
                dout ( 8 DOWNTO  0) <= bank0 (CONV_INTEGER (addr_read));
                dout (17 DOWNTO  9) <= bank1 (CONV_INTEGER (addr_read));
                dout (26 DOWNTO 18) <= bank2 (CONV_INTEGER (addr_read));
                dout (35 DOWNTO 27) <= bank3 (CONV_INTEGER (addr_read));
            ELSE
                dout <= (OTHERS => 'Z');
            END IF;
        END IF;
